...
wget https://github.com/reey/PKGBUILDs/releases/download/v4.20.2/linux-nyan-4.20.2-1-armv7h.pkg.tar.xz
wget https://github.com/reey/PKGBUILDs/releases/download/v4.20.2/linux-nyan-headers-4.20.2-1-armv7h.pkg.tar.xz
...
start_progress "Installing Das U-Boot"
PARTUUID="$(sudo blkid -s PARTUUID -o value /dev/mmcblk1p2)"
cat > ${MY_CHROOT_DIR}/install-uboot.sh << EOF
#install u-boot
wget https://github.com/reey/PKGBUILDs/releases/download/uboot-2018.11/uboot-nyan-big-2018.11-1-armv7h.pkg.tar.xz
yes n | pacman -U --noconfirm uboot-nyan-big-*
dd if=/boot/u-boot.kpart of=${target_uboot}

#mkdir /extlinux
#cd /extlinux
#wget https://raw.githubusercontent.com/reey/LinuxOnAcerCB5-311/mainline/extlinux.conf
#head -c -1 extlinux.conf > extlinux.conf.clean
#echo "$(blkid -s PARTUUID -o value ${target_rootfs})" >> extlinux.conf.clean
#mv extlinux.conf.clean extlinux.conf

EOF

exec_in_chroot install-uboot.sh
...
if [ "$1" != "" ]; then
  ...
  cgpt add -i ${uboot_part} -b 64 -s 32768 -S 1 -P 1 -l UBOOT -t kernel ${target_disk}
  cgpt add -i ${root_part} -b 65600 -s $aroot_size -l ROOT -t rootfs ${target_disk}
  sync
  ...
  ckern_size="`cgpt show -i ${uboot_part} -n -s -q ${target_disk}`"
  ...
  then
    ...
    # now kernc
    cgpt add -i ${uboot_part} -b $kernc_start -s $kernc_size -l KERN-C ${target_disk}
...
then
  ...
  target_uboot="${target_disk}p${uboot_part}"
else
  target_rootfs="${target_disk}${root_part}"
  target_uboot="${target_disk}${uboot_part}"
fi

echo "Target Kernel Partition: ${target_uboot}  Target Root FS: ${target_rootfs}"
